...
vmlinux-dirs	:= $(patsubst %/,%,$(filter %/, $(init-y) $(init-m) \
		     ...
		     $(net-y) $(net-m) $(libs-y) $(libs-m) $(virt-y) $(uwvg-y) $(uwvg-m) ))

vmlinux-alldirs	:= $(sort $(vmlinux-dirs) $(patsubst %/,%,$(filter %/, \
		     $(init-) $(core-) $(drivers-) $(net-) $(libs-) $(virt-) $(uwvg-))))
...
uwvg-y          := $(patsubst %/, %/built-in.o, $(uwvg-y))
# Externally visible symbols (used by link-vmlinux.sh)
export KBUILD_VMLINUX_INIT := $(head-y) $(init-y)
export KBUILD_VMLINUX_MAIN := $(core-y) $(libs-y) $(drivers-y) $(net-y) $(virt-y) $(uwvg-y)
